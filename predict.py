...
def seq2crops(seq, step=1):
    ...
    for path, img, im0s, vid_cap in seq:
        ...
        pred = torch.tensor(np.array(sess.run([sess.get_outputs()[0].name], {sess.get_inputs()[0].name: img})))
...
def crops2latents(crops):
    ...
    p = lambda img: sess.run([sess.get_outputs()[0].name],
                             {sess.get_inputs()[0].name: img.astype('float32').reshape((1, 3, 32, 32)) / 255.0})[0]
    res = []
    for frame in crops:
        arr = p(frame[0])
        for crop in frame[1:]:
...
def latents2pred(latents):
    ...
    if (len(latents) == 0):
        print("None")
        return -1
    pred = sess.run([sess.get_outputs()[0].name], {sess.get_inputs()[0].name: latents.reshape((1, len(latents), 64))})[
        0]
